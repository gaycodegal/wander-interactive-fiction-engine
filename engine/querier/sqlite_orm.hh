...
inline std::error_code make_error_code(sqlite_orm::orm_error_code errorCode) {
  return std::error_code(static_cast<int>(errorCode),
...
template <typename T, typename F> auto static_if(std::true_type, T t, F) {
  return std::move(t);
}

template <typename T, typename F> auto static_if(std::false_type, T, F f) {
  return std::move(f);
}

template <bool B, typename T, typename F> auto static_if(T t, F f) {
  return static_if(std::integral_constant<bool, B>{}, std::move(t),
                   std::move(f));
}

template <bool B, typename T> auto static_if(T t) {
...
template <size_t N, class... Args> struct iterator {
  ...
  template <class L>
  void operator()(const std::tuple<Args...> &t, const L &l,
                  bool reverse = true) {
    if (reverse) {
      l(std::get<N>(t));
...
void move_tuple_impl(L &lhs, R &rhs) {
  ...
  internal::static_if<std::integral_constant<bool, N != I + 1>{}>(
      [](auto &lhs, auto &rhs) { move_tuple_impl<N, I + 1>(lhs, rhs); })(lhs,
                                                                         rhs);
}
} // namespace tuple_helper
...
struct autoincrement_t {
  ...
  operator std::string() const { return "AUTOINCREMENT"; }
};
...
template <class F> struct on_update_delete_t : on_update_delete_base {
  ...
  on_update_delete_t(decltype(fk) fk_, decltype(update) update,
...
struct foreign_key_t<std::tuple<Cs...>, std::tuple<Rs...>> {
  ...
  foreign_key_t(const self &other)
      : columns(other.columns), references(other.references),
  ...
  template <class... Opts> constexpr bool has_every() const { return false; }
};
...
template <class... Cs> struct foreign_key_intermediate_t {
  ...
  template <class... Rs>
  foreign_key_t<std::tuple<Cs...>, std::tuple<Rs...>>
  references(Rs... references) {
    return {std::move(this->columns),
            std::make_tuple(std::forward<Rs>(references)...)};
  }
};
#endif
...
template <> struct is_constraint<unique_t> : std::true_type {};
 ...
 */
inline constraints::unique_t unique() { return {}; }

inline constraints::autoincrement_t autoincrement() { return {}; }
...
struct type_is_nullable<std::optional<T>> : public std::true_type {
  bool operator()(const std::optional<T> &t) const { return t.has_value(); }
};
#endif // SQLITE_ORM_OPTIONAL_SUPPORTED
 ...
 */
template <class T> internal::expression_t<T> c(T t) { return {std::move(t)}; }
 ...
 */
template <class O, class T, class G /* = const T& (O::*)() const*/,
          class S /* = void (O::*)(T)*/, class... Op>
struct column_t : column_base {
  ...
  column_t(std::string name, member_pointer_t member_pointer_,
           getter_type getter_, setter_type setter_,
           constraints_type constraints_)
      : column_base{std::move(name)}, member_pointer(member_pointer_),
        getter(getter_), setter(setter_), constraints(std::move(constraints_)) {
  }

  /**
   *  Simplified interface for `NOT NULL` constraint
   */
  bool not_null() const { return !type_is_nullable<field_type>::value; }
  ...
  template <class O1> constexpr bool has_every() const { return has<O1>(); }
...
make_column(const std::string &name, T O::*m, Op... constraints) {
  static_assert(constraints::constraints_size<Op...>::value ==
...
internal::column_t<typename internal::setter_traits<S>::object_type,
                   typename internal::setter_traits<S>::field_type, G, S, Op...>
make_column(const std::string &name, S setter, G getter, Op... constraints) {
  static_assert(
      std::is_same<typename internal::setter_traits<S>::field_type,
...
template <> struct field_printer<std::nullptr_t> {
  std::string operator()(const std::nullptr_t &) const { return "null"; }
};
...
template <class T> struct optional_container {
  ...
  template <class L> void apply(const L &l) const { l(this->field); }
};
...
struct is_equal_t : binary_condition<L, R>, is_equal_string {
  ...
  negated_condition_t<self> operator!() const { return {*this}; }
 ...
 *  IN operator object.
 */
template <class L, class A> struct in_t : condition_t, in_base {
  using self = in_t<L, A>;

  L l;   //  left expression
  A arg; //  in arg

  in_t(L l_, A arg_, bool negative)
      : in_base{negative}, l(l_), arg(std::move(arg_)) {}
 ...
 */
template <class T> struct is_null_t : is_null_string {
 ...
 */
template <class T> struct is_not_null_t : is_not_null_string {
...
struct where_string {
  operator std::string() const { return "WHERE"; }
};
 ...
 *  S - storage class
 */
template <class S> struct dynamic_order_by_t : order_by_string {
  ...
  template <class O> void push_back(order_by_t<O> order_by) {
    ...
    entries.emplace_back(move(columnName), order_by.asc_desc,
                         move(order_by._collate_argument));
  }
 ...
 */
template <class A, class T, class E> struct like_t : condition_t, like_string {
  ...
  like_t(arg_t arg_, pattern_t pattern_,
         sqlite_orm::internal::optional_container<escape_t> escape)
      : arg(std::move(arg_)), pattern(std::move(pattern_)),
        arg3(std::move(escape)) {}
...
template <class A, class T> struct glob_t : condition_t, glob_string {
...
struct exists_string {
  operator std::string() const { return "EXISTS"; }
};

template <class T> struct exists_t : condition_t, exists_string {
  using type = T;
...
struct having_string {
  operator std::string() const { return "HAVING"; }
};
...
template <class T, class R>
conditions::lesser_or_equal_t<T, R> operator<=(internal::expression_t<T> expr,
                                               R r) {
  return {std::move(expr.t), std::move(r)};
}

template <class L, class T>
conditions::lesser_or_equal_t<L, T> operator<=(L l,
                                               internal::expression_t<T> expr) {
  return {std::move(l), std::move(expr.t)};
}

template <class T, class R>
conditions::greater_than_t<T, R> operator>(internal::expression_t<T> expr,
                                           R r) {
  return {std::move(expr.t), std::move(r)};
}

template <class L, class T>
conditions::greater_than_t<L, T> operator>(L l,
                                           internal::expression_t<T> expr) {
  return {std::move(l), std::move(expr.t)};
}

template <class T, class R>
conditions::greater_or_equal_t<T, R> operator>=(internal::expression_t<T> expr,
                                                R r) {
  return {std::move(expr.t), std::move(r)};
}

template <class L, class T>
conditions::greater_or_equal_t<L, T>
...
template <class T, class R>
conditions::is_not_equal_t<T, R> operator!=(internal::expression_t<T> expr,
                                            R r) {
  return {std::move(expr.t), std::move(r)};
}

template <class L, class T>
conditions::is_not_equal_t<L, T> operator!=(L l,
                                            internal::expression_t<T> expr) {
  return {std::move(l), std::move(expr.t)};
}
...
template <class L, class R>
internal::conc_t<L, R> operator||(internal::expression_t<L> l,
                                  internal::expression_t<R> r) {
...
template <class L, class R>
internal::add_t<L, R> operator+(internal::expression_t<L> l,
                                internal::expression_t<R> r) {
  return {std::move(l.t), std::move(r.t)};
}
...
template <class L, class R>
internal::sub_t<L, R> operator-(internal::expression_t<L> l,
                                internal::expression_t<R> r) {
  return {std::move(l.t), std::move(r.t)};
}
...
template <class L, class R>
internal::mul_t<L, R> operator*(internal::expression_t<L> l,
                                internal::expression_t<R> r) {
  return {std::move(l.t), std::move(r.t)};
}
...
template <class L, class R>
internal::div_t<L, R> operator/(internal::expression_t<L> l,
                                internal::expression_t<R> r) {
  return {std::move(l.t), std::move(r.t)};
}
...
template <class L, class R>
internal::mod_t<L, R> operator%(internal::expression_t<L> l,
                                internal::expression_t<R> r) {
  return {std::move(l.t), std::move(r.t)};
}

template <class F, class O> conditions::using_t<F, O> using_(F O::*p) {
  return {std::move(p)};
}

template <class T> conditions::on_t<T> on(T t) { return {std::move(t)}; }

template <class T> conditions::cross_join_t<T> cross_join() { return {}; }

template <class T> conditions::natural_join_t<T> natural_join() { return {}; }

template <class T, class O> conditions::left_join_t<T, O> left_join(O o) {
  return {std::move(o)};
}

template <class T, class O> conditions::join_t<T, O> join(O o) {
  return {std::move(o)};
}
...
template <class T, class O> conditions::inner_join_t<T, O> inner_join(O o) {
  return {std::move(o)};
}

template <class T> conditions::offset_t<T> offset(T off) {
  return {std::move(off)};
}

template <class T> conditions::limit_t<T, false, false, void> limit(T lim) {
  return {std::move(lim)};
}

template <class T, class O>
typename std::enable_if<!conditions::is_offset<T>::value,
                        conditions::limit_t<T, true, true, O>>::type
limit(O off, T lim) {
  return {std::move(lim), {std::move(off)}};
}

template <class T, class O>
conditions::limit_t<T, true, false, O> limit(T lim,
                                             conditions::offset_t<O> offt) {
  return {std::move(lim), {std::move(offt.off)}};
}

template <class L, class R,
          typename = typename std::enable_if<
              std::is_base_of<conditions::condition_t, L>::value ||
...
template <class T> conditions::is_not_null_t<T> is_not_null(T t) {
  return {std::move(t)};
}

template <class T> conditions::is_null_t<T> is_null(T t) {
  return {std::move(t)};
}
...
template <class L, class A> conditions::in_t<L, A> in(L l, A arg) {
...
template <class L, class E>
conditions::in_t<L, std::vector<E>> not_in(L l,
                                           std::initializer_list<E> values) {
  return {std::move(l), std::move(values), true};
}

template <class L, class A> conditions::in_t<L, A> not_in(L l, A arg) {
  return {std::move(l), std::move(arg), true};
}

template <class L, class R> conditions::is_equal_t<L, R> is_equal(L l, R r) {
  return {std::move(l), std::move(r)};
}

template <class L, class R> conditions::is_equal_t<L, R> eq(L l, R r) {
  return {std::move(l), std::move(r)};
}
...
template <class L, class R> conditions::is_not_equal_t<L, R> ne(L l, R r) {
  return {std::move(l), std::move(r)};
}
...
template <class L, class R> conditions::greater_than_t<L, R> gt(L l, R r) {
  return {std::move(l), std::move(r)};
}
...
template <class L, class R> conditions::greater_or_equal_t<L, R> ge(L l, R r) {
  return {std::move(l), std::move(r)};
}
...
template <class L, class R> conditions::lesser_than_t<L, R> lt(L l, R r) {
  return {std::move(l), std::move(r)};
}
...
template <class L, class R> conditions::lesser_or_equal_t<L, R> le(L l, R r) {
  return {std::move(l), std::move(r)};
}

template <class C> conditions::where_t<C> where(C c) { return {std::move(c)}; }

/**
 * ORDER BY column
 * Example: storage.select(&User::name, order_by(&User::id))
 */
template <class O> conditions::order_by_t<O> order_by(O o) {
  return {std::move(o)};
}
 ...
 */
template <class A, class T> conditions::like_t<A, T, void> like(A a, T t) {
 ...
 */
template <class A, class T> conditions::glob_t<A, T> glob(A a, T t) {
  return {std::move(a), std::move(t)};
}
 ...
 */
template <class T> conditions::exists_t<T> exists(T t) {
  return {std::move(t)};
}
 ...
 */
template <class T> conditions::having_t<T> having(T t) {
  return {std::move(t)};
}
 ...
 *  Example: cast<std::string>(&User::id)
 */
template <class T, class E> conditions::cast_t<T, E> cast(E e) {
  return {std::move(e)};
}
} // namespace sqlite_orm
#pragma once
...
struct alias_extractor<
    T, typename std::enable_if<std::is_base_of<alias_tag, T>::value>::type> {
...
template <class T> internal::alias_holder<T> get() { return {}; }
...
struct join_iterator<H, Tail...> : public join_iterator<Tail...> {
  ...
  template <class L> void operator()(const L &l) { this->super::operator()(l); }
};

template <class T, class... Tail>
struct join_iterator<conditions::cross_join_t<T>, Tail...>
...
template <class T, class... Tail>
struct join_iterator<conditions::natural_join_t<T>, Tail...>
    : public join_iterator<Tail...> {
  using super = join_iterator<Tail...>;
...
struct join_iterator<conditions::left_join_t<T, O>, Tail...>
    : public join_iterator<Tail...> {
  using super = join_iterator<Tail...>;
...
template <class T, class O, class... Tail>
struct join_iterator<conditions::join_t<T, O>, Tail...>
    : public join_iterator<Tail...> {
  using super = join_iterator<Tail...>;
...
template <class T, class O, class... Tail>
struct join_iterator<conditions::left_outer_join_t<T, O>, Tail...>
    : public join_iterator<Tail...> {
  using super = join_iterator<Tail...>;
...
template <class T, class O, class... Tail>
struct join_iterator<conditions::inner_join_t<T, O>, Tail...>
    : public join_iterator<Tail...> {
  using super = join_iterator<Tail...>;
...
struct length_string {
  operator std::string() const { return "LENGTH"; }
};

struct abs_string {
  operator std::string() const { return "ABS"; }
};
...
struct changes_string {
  operator std::string() const { return "CHANGES"; }
};

struct trim_string {
  operator std::string() const { return "TRIM"; }
};

struct ltrim_string {
  operator std::string() const { return "LTRIM"; }
};

struct rtrim_string {
  operator std::string() const { return "RTRIM"; }
};

#if SQLITE_VERSION_NUMBER >= 3007016

struct char_string {
  operator std::string() const { return "CHAR"; }
};

struct random_string {
  operator std::string() const { return "RANDOM"; }
};

#endif

struct coalesce_string {
  operator std::string() const { return "COALESCE"; }
};
...
struct datetime_string {
  operator std::string() const { return "DATETIME"; }
};

struct julianday_string {
  operator std::string() const { return "JULIANDAY"; }
};

struct zeroblob_string {
  operator std::string() const { return "ZEROBLOB"; }
};

struct substr_string {
  operator std::string() const { return "SUBSTR"; }
};
} // namespace core_functions
...
template <class F, class R,
          typename = typename std::enable_if<internal::is_base_of_template<
              F, core_functions::core_function_t>::value>::type>
 ...
 */
template <class T>
core_functions::core_function_t<int, core_functions::length_string, T>
length(T t) {
 ...
 */
template <class T>
core_functions::core_function_t<std::unique_ptr<double>,
                                core_functions::abs_string, T>
abs(T t) {
  std::tuple<T> args{std::forward<T>(t)};
  return {std::move(args)};
}
 ...
 */
template <class T>
core_functions::core_function_t<std::string, core_functions::lower_string, T>
lower(T t) {
  std::tuple<T> args{std::forward<T>(t)};
  return {std::move(args)};
}
 ...
 */
template <class T>
core_functions::core_function_t<std::string, core_functions::upper_string, T>
upper(T t) {
  std::tuple<T> args{std::forward<T>(t)};
  return {std::move(args)};
}
 ...
 */
inline core_functions::core_function_t<int, core_functions::changes_string>
changes() {
  return {{}};
}
 ...
 */
template <class T>
core_functions::core_function_t<std::string, core_functions::trim_string, T>
trim(T t) {
  std::tuple<T> args{std::forward<T>(t)};
  return {std::move(args)};
}
...
core_functions::core_function_t<std::string, core_functions::trim_string, X, Y>
trim(X x, Y y) {
 ...
 */
template <class X>
core_functions::core_function_t<std::string, core_functions::ltrim_string, X>
ltrim(X x) {
 ...
 */
template <class X, class Y>
core_functions::core_function_t<std::string, core_functions::ltrim_string, X, Y>
ltrim(X x, Y y) {
  std::tuple<X, Y> args{std::forward<X>(x), std::forward<Y>(y)};
  return {std::move(args)};
}
 ...
 */
template <class X>
core_functions::core_function_t<std::string, core_functions::rtrim_string, X>
rtrim(X x) {
  std::tuple<X> args{std::forward<X>(x)};
  return {std::move(args)};
}
 ...
 */
template <class X, class Y>
core_functions::core_function_t<std::string, core_functions::rtrim_string, X, Y>
rtrim(X x, Y y) {
  std::tuple<X, Y> args{std::forward<X>(x), std::forward<Y>(y)};
  return {std::move(args)};
}
 ...
 */
template <class... Args>
core_functions::core_function_t<std::string, core_functions::char_string,
                                Args...>
char_(Args... args) {
  return {std::make_tuple(std::forward<Args>(args)...)};
}
 ...
 */
inline core_functions::core_function_t<int, core_functions::random_string>
random() {
  return {{}};
}
...
core_functions::core_function_t<R, core_functions::coalesce_string, Args...>
coalesce(Args... args) {
  return {std::make_tuple(std::forward<Args>(args)...)};
}
 ...
 */
template <class... Args>
core_functions::core_function_t<std::string, core_functions::date_string,
                                Args...>
date(Args... args) {
 ...
 */
template <class... Args>
core_functions::core_function_t<std::string, core_functions::datetime_string,
                                Args...>
datetime(Args... args) {
  std::tuple<Args...> t{std::forward<Args>(args)...};
  return {std::move(t)};
 ...
 */
template <class... Args>
core_functions::core_function_t<double, core_functions::julianday_string,
                                Args...>
julianday(Args... args) {
  std::tuple<Args...> t{std::forward<Args>(args)...};
  return {std::move(t)};
 ...
 */
template <class N>
core_functions::core_function_t<std::vector<char>,
                                core_functions::zeroblob_string, N>
zeroblob(N n) {
 ...
 */
template <class X, class Y>
core_functions::core_function_t<std::string, core_functions::substr_string, X,
                                Y>
substr(X x, Y y) {
  std::tuple<X, Y> args{std::forward<X>(x), std::forward<Y>(y)};
  return {std::move(args)};
...
core_functions::core_function_t<std::string, core_functions::substr_string, X,
                                Y, Z>
substr(X x, Y y, Z z) {
  std::tuple<X, Y, Z> args{std::forward<X>(x), std::forward<Y>(y),
                           std::forward<Z>(z)};
  return {std::move(args)};
}
...
struct avg_string {
  operator std::string() const { return "AVG"; }
};
 ...
 */
template <class T> struct avg_t : avg_string {
  using arg_type = T;
...
struct count_string {
  operator std::string() const { return "COUNT"; }
};
 ...
 *  T is an argument type
 */
template <class T> struct count_t : count_string {
  using arg_type = T;
...
struct sum_string {
  operator std::string() const { return "SUM"; }
};
 ...
 *  T is an argument type
 */
template <class T> struct sum_t : sum_string {
  using arg_type = T;
...
struct total_string {
  operator std::string() const { return "TOTAL"; }
};
 ...
 *  T is an argument type
 */
template <class T> struct total_t : total_string {
  using arg_type = T;
...
struct max_string {
  operator std::string() const { return "MAX"; }
};
 ...
 *  T is an argument type
 */
template <class T> struct max_t : max_string {
  using arg_type = T;
...
struct min_string {
  operator std::string() const { return "MIN"; }
};
 ...
 *  T is an argument type
 */
template <class T> struct min_t : min_string {
  using arg_type = T;
...
struct group_concat_string {
  operator std::string() const { return "GROUP_CONCAT"; }
};
 ...
 *  T is an argument type
 */
template <class T> struct group_concat_single_t : group_concat_string {
  using arg_type = T;
 ...
 *  T is an argument type
 */
template <class T> struct group_concat_double_t : group_concat_double_base {
  using arg_type = T;
 ...
 */
template <class T> aggregate_functions::avg_t<T> avg(T t) {
  return {std::move(t)};
}
 ...
 */
template <class T> aggregate_functions::count_t<T> count(T t) {
  return {std::move(t)};
}
 ...
 */
inline aggregate_functions::count_asterisk_without_type count() { return {}; }

/**
 *  COUNT(*) with FROM function. Specified type T will be serializeed as
 *  a from argument.
 */
template <class T> aggregate_functions::count_asterisk_t<T> count() {
  return {};
}
 ...
 */
template <class T> aggregate_functions::sum_t<T> sum(T t) {
  return {std::move(t)};
}
 ...
 */
template <class T> aggregate_functions::max_t<T> max(T t) {
  return {std::move(t)};
}
 ...
 */
template <class T> aggregate_functions::min_t<T> min(T t) {
  return {std::move(t)};
}
 ...
 */
template <class T> aggregate_functions::total_t<T> total(T t) {
  return {std::move(t)};
}
...
template <class L, class R> struct typed_comparator {
  bool operator()(const L &, const R &) const { return false; }
};

template <class O> struct typed_comparator<O, O> {
  bool operator()(const O &lhs, const O &rhs) const { return lhs == rhs; }
};
...
struct set_string {
  operator std::string() const { return "SET"; }
};
 ...
 */
template <class T> bool get_distinct(const T &) { return false; }
...
template <class T> internal::then_t<T> then(T t) { return {std::move(t)}; }
...
template <class T> internal::all_t<T> all(T t) { return {std::move(t)}; }
...
template <class T> internal::asterisk_t<T> asterisk() { return {}; }
} // namespace sqlite_orm
#pragma once
...
struct statement_finalizer {
  ...
  inline ~statement_finalizer() { sqlite3_finalize(this->stmt); }
};
} // namespace sqlite_orm
#pragma once
...
template <typename T> struct is_std_ptr<std::shared_ptr<T>> : std::true_type {
  ...
  static std::shared_ptr<T> make(const T &v) { return std::make_shared<T>(v); }
};

template <typename T> struct is_std_ptr<std::unique_ptr<T>> : std::true_type {
  using element_type = T;

  static std::unique_ptr<T> make(const T &v) { return std::make_unique<T>(v); }
};
} // namespace sqlite_orm
...
struct statement_binder<
    V, std::enable_if_t<std::is_same<V, std::string>::value ||
                        std::is_same<V, const char *>::value>> {
  int bind(sqlite3_stmt *stmt, int index, const V &value) {
    return sqlite3_bind_text(stmt, index, string_data(value), -1,
                             SQLITE_TRANSIENT);
  }

private:
  const char *string_data(const std::string &s) const { return s.c_str(); }

  const char *string_data(const char *s) const { return s; }
};
...
template <class V>
struct statement_binder<
    V, std::enable_if_t<std::is_same<V, std::wstring>::value ||
...
using is_bindable = std::integral_constant<
    bool, !std::is_base_of<std::false_type, statement_binder<T>>::value>;
...
struct conditional_binder<T, std::false_type> : conditional_binder_base {
  ...
  int operator()(const T &) const { return SQLITE_OK; }
};
...
struct bindable_filter_single<
    T, typename std::enable_if<is_bindable<T>::value>::type> {
...
struct row_extractor<V, std::enable_if_t<std::is_arithmetic<V>::value>> {
  V extract(const char *row_value) { return extract(row_value, tag()); }
...
struct row_extractor<
    V, std::enable_if_t<std::is_same<V, std::vector<char>>::value>> {
...
template <class... Cols> struct index_t {
  ...
  template <class L> void for_each_column_with_constraints(const L &) {}
};
} // namespace internal
...
template <class... Cols>
internal::index_t<Cols...> make_unique_index(const std::string &name,
                                             Cols... cols) {
...
template <class T> internal::table_rowid_t<T> rowid() { return {}; }

template <class T> internal::table_oid_t<T> oid() { return {}; }

template <class T> internal::table__rowid_t<T> _rowid_() { return {}; }
} // namespace sqlite_orm
#pragma once
 ...
 * for core_functions::length_t has `type` int cause LENGTH returns INTEGER in
 * sqlite. Every column_result_t must have `type` type that equals c++ SELECT
 * return type for T T - C++ type SFINAE - sfinae argument
 */
...
struct column_result_t<St, T,
                       typename std::enable_if<is_base_of_template<
                           T, core_functions::core_function_t>::value>::type> {
...
template <class St, class T>
struct column_result_t<St, aggregate_functions::group_concat_single_t<T>,
                       void> {
  using type = std::string;
};

template <class St, class T>
struct column_result_t<St, aggregate_functions::group_concat_double_t<T>,
                       void> {
  using type = std::string;
};
...
template <class St>
struct column_result_t<St, aggregate_functions::count_asterisk_without_type,
                       void> {
  using type = int;
};
...
struct column_result_t<St, T,
                       typename std::enable_if<is_base_of_template<
                           T, compound_operator>::value>::type> {
...
template <class St, class T>
struct column_result_t<
    St, T, typename std::enable_if<std::is_arithmetic<T>::value>::type> {
  using type = T;
};
...
template <class T, class... Cs> struct table_t : table_base {
   ...
   * columns with table constraints use for_each_column_with_constraints
   * instead. L is lambda type. Do not specify it explicitly.
  ...
  template <class L> void for_each_column_with_constraints(const L &l) const {
  ...
  template <class Op, class L> void for_each_column_with(const L &l) const {
    ...
    iterate_tuple(this->columns, [&l](auto &column) {
      using column_type = typename std::decay<decltype(column)>::type;
      static_if<
          tuple_contains_type<Op, typename column_type::constraints_type>{}>(l)(
          column);
    });
  }
 ...
 */
template <class... Cs, class T = typename std::tuple_element<
                           0, std::tuple<Cs...>>::type::object_type>
...
struct field_value_holder<
    T, typename std::enable_if<getter_traits<T>::returns_lvalue>::type> {
  using type = typename getter_traits<T>::field_type;
...
struct storage_impl_base {
  ...
  std::vector<table_info> get_table_info(const std::string &tableName,
                                         sqlite3 *db) {
    std::vector<table_info> res;
...
struct storage_impl<H, Ts...> : public storage_impl<Ts...> {
  ...
  /**
   *  Cute function used to find column name by its type and getter pointer.
   * Uses SFINAE to skip inequal type O.
   */
  template <class O, class F, class HH = typename H::object_type>
  std::string
  column_name(const F &(O::*g)() const,
              typename std::enable_if<std::is_same<O, HH>::value>::type * =
                  nullptr) const {
    return this->table.find_column_name(g);
  }
  ...
  /**
   *  Cute function used to find column name by its type and setter pointer.
   * Uses SFINAE to skip inequal type O.
   */
  template <class O, class F, class HH = typename H::object_type>
  std::string
  column_name(void (O::*s)(F),
              typename std::enable_if<std::is_same<O, HH>::value>::type * =
                  nullptr) const {
    return this->table.find_column_name(s);
  }
  ...
  auto &get_impl(typename std::enable_if<std::is_same<O, HH>::value>::type * =
                     nullptr) const {
    return *this;
  }
  ...
  template <class O, class HH = typename H::object_type>
  std::string
  find_table_name(typename std::enable_if<std::is_same<O, HH>::value>::type * =
                      nullptr) const {
  ...
  void copy_table(sqlite3 *db, const std::string &name) {
    ...
    this->table.for_each_column(
        [&columnNames](auto &c) { columnNames.emplace_back(c.name); });
  ...
  sync_schema_result schema_status(sqlite3 *db, bool preserve) {
    ...
    if (!gottaCreateTable) {
      ...
      if (this->get_remove_add_columns(columnsToAdd, storageTableInfo,
                                       dbTableInfo)) {
...
template <> struct storage_impl<> : storage_impl_base {
  ...
  template <class O> std::string find_table_name() const { return {}; }

  template <class L> void for_each(const L &) {}

  int foreign_keys_count() { return 0; }

  template <class O> std::string dump(const O &, sqlite3 *, std::nullptr_t) {
    throw std::system_error(
        std::make_error_code(orm_error_code::type_is_not_mapped_to_storage));
  }
};
...
struct connection_holder {
  ...
  void retain() {
    ...
    if (1 == this->_retain_count) {
      ...
      if (rc != SQLITE_OK) {
        throw std::system_error(std::error_code(sqlite3_errcode(this->db),
...
template <class T, class... Args> struct get_all_t {
  using type = T;
...
template <class T, class... Args> struct get_all_pointer_t {
  using type = T;
...
#ifdef SQLITE_ORM_OPTIONAL_SUPPORTED
template <class T, class... Args> struct get_all_optional_t {
  using type = T;
...
template <class T, class SFINAE = void> struct ast_iterator {
   ...
   */
  template <class L> void operator()(const T &t, const L &l) const { l(t); }
};
...
template <class C> struct ast_iterator<conditions::where_t<C>, void> {
...
template <class T>
struct ast_iterator<T, typename std::enable_if<is_base_of_template<
                           T, conditions::binary_condition>::value>::type> {
  using node_type = T;
...
template <class L, class A> struct ast_iterator<conditions::in_t<L, A>, void> {
...
template <class T> struct ast_iterator<conditions::having_t<T>, void> {
...
template <class T> struct ast_iterator<conditions::exists_t<T>, void> {
...
template <class T> struct ast_iterator<conditions::named_collate<T>, void> {
...
template <class T> struct ast_iterator<conditions::is_null_t<T>, void> {
...
template <class T> struct ast_iterator<conditions::is_not_null_t<T>, void> {
...
template <class T> struct ast_iterator<conditions::on_t<T>, void> {
...
struct ast_iterator<simple_case_t<R, T, E, Args...>, void> {
  ...
  template <class L> void operator()(const node_type &c, const L &l) const {
    c.case_expression.apply([&l](auto &c) { iterate_ast(c, l); });
...
template <class T, class S, class... Args> struct view_t {
  ...
  view_t(storage_type &stor, decltype(connection) conn, Args &&... args_)
      : storage(stor),
        connection(std::move(conn)), args{std::make_tuple(
                                         std::forward<Args>(args_)...)} {}

  size_t size() { return this->storage.template count<T>(); }

  bool empty() { return !this->size(); }

  iterator_t<self> end() { return {nullptr, *this}; }
  ...
  iterator_t<self> begin() {
    ...
    if (ret == SQLITE_OK) {
      ...
      iterate_ast(this->args.conditions, [&index, stmt, db](auto &node) {
        ...
        conditional_binder<node_type, is_bindable<node_type>> binder{stmt,
                                                                     index};
...
struct backup_t {
  ...
  backup_t(backup_t &&other)
      : handle(other.handle), holder(move(other.holder)), to(other.to),
        from(other.from) {
...
struct storage_base {
  ...
  void create_collation(const std::string &name, collating_function f) {
    ...
    if (this->connection->retain_count() > 0) {
      ...
      if (sqlite3_create_collation(
              db, name.c_str(), SQLITE_UTF8, functionPointer,
  ...
  backup_t make_backup_to(const std::string &filename) {
    ...
    return {connection_ref{*holder}, "main", this->get_connection(), "main",
            move(holder)};
  }
  ...
  backup_t make_backup_from(const std::string &filename) {
    auto holder = std::make_unique<connection_holder>(filename);
    return {this->get_connection(), "main", connection_ref{*holder}, "main",
            move(holder)};
  }
...
protected:
  ...
  void on_open_internal(sqlite3 *db) {
    ...
    for (auto &p : this->collatingFunctions) {
      if (sqlite3_create_collation(db, p.first.c_str(), SQLITE_UTF8, &p.second,
  ...
  template <class S>
  std::string
  process_order_by(const conditions::dynamic_order_by_t<S> &orderBy) const {
...
template <class T> struct get_ref_t {
  ...
  template <class O> auto &operator()(O &t) const { return t; }
};

template <class T> struct get_ref_t<std::reference_wrapper<T>> {

  template <class O> auto &operator()(O &t) const { return t.get(); }
};
...
template <class T> struct get_object_t<replace_t<T>> {
  ...
  template <class O> auto &operator()(O &e) const { return get_ref(e.obj); }
};
...
template <class S> struct dynamic_order_by_t;
}
...
protected:
  ...
  template <class S> friend struct conditions::dynamic_order_by_t;

  template <class V> friend struct iterator_t;

  template <class O, class T, class G, class S, class... Op>
  std::string
  serialize_column_schema(const internal::column_t<O, T, G, S, Op...> &c) {
    std::stringstream ss;
    ss << "'" << c.name << "' ";
    using column_type = typename std::decay<decltype(c)>::type;
    using field_type = typename column_type::field_type;
    ...
    ss << type_printer<field_type>().print() << " ";
    {
      ...
      int tupleIndex = 0;
      iterate_tuple(c.constraints, [&constraintsStrings, &primaryKeyIndex,
                                    &autoincrementIndex, &tupleIndex](auto &v) {
        ...
        } else if (std::is_same<constraints::autoincrement_t,
  ...
  template <class... Cs>
  std::string
  serialize_column_schema(const constraints::primary_key_t<Cs...> &fk) {
    std::stringstream ss;
  ...
  std::string serialize_column_schema(
      const constraints::foreign_key_t<std::tuple<Cs...>, std::tuple<Rs...>>
          &fk) {
    std::stringstream ss;
    std::vector<std::string> columnNames;
    ...
    referencesNames.reserve(referencesCount);
    {
      ...
      auto refTableName =
          this->impl.template find_table_name<first_reference_mapped_type>();
      ss << refTableName << " ";
    }
  ...
  void create_table(sqlite3 *db, const std::string &tableName, I *impl) {
    ...
    auto index = 0;
    impl->table.for_each_column_with_constraints(
        [columnsCount, &index, &ss, this](auto &c) {
  ...
  template <class I> void backup_table(sqlite3 *db, I *impl) {
  ...
  template <class O> void assert_mapped_type() const {
    ...
    static_assert(tuple_helper::has_type<O, mapped_types_tuples>::value,
  ...
  template <class T>
  std::string string_from_expression(std::reference_wrapper<T> ref,
                                     bool noTableName) const {
    return this->string_from_expression(ref.get(), noTableName);
  }

  std::string string_from_expression(std::nullptr_t,
                                     bool /*noTableName*/) const {
    return "?";
  }

  template <class T>
  std::string string_from_expression(const alias_holder<T> &,
                                     bool /*noTableName*/) const {
    return T::get();
  }

  template <class R, class S, class... Args>
  std::string string_from_expression(
      const core_functions::core_function_t<R, S, Args...> &c,
      bool noTableName) const {
    std::stringstream ss;
  ...
  template <class T, class E>
  std::string string_from_expression(const as_t<T, E> &als,
                                     bool noTableName) const {
    auto tableAliasString = alias_extractor<T>::get();
    return this->string_from_expression(als.expression, noTableName) + " AS " +
           tableAliasString;
  }

  template <class T, class C>
  std::string string_from_expression(const alias_column_t<T, C> &als,
                                     bool noTableName) const {
    std::stringstream ss;
    if (!noTableName) {
      ss << "'" << T::get() << "'.";
  ...
  std::string string_from_expression(const std::string &,
                                     bool /*noTableName*/) const {
    return "?";
  }
  ...
  std::string string_from_expression(const rowid_t &rid,
                                     bool /*noTableName*/) const {
    return static_cast<std::string>(rid);
  }

  std::string string_from_expression(const oid_t &rid,
                                     bool /*noTableName*/) const {
    return static_cast<std::string>(rid);
  }
  ...
  template <class O>
  std::string string_from_expression(const table_rowid_t<O> &rid,
                                     bool noTableName) const {
    std::stringstream ss;
    if (!noTableName) {
      ss << "'" << this->impl.template find_table_name<O>() << "'.";
  ...
  template <class O>
  std::string string_from_expression(const table_oid_t<O> &rid,
                                     bool noTableName) const {
    std::stringstream ss;
    if (!noTableName) {
      ss << "'" << this->impl.template find_table_name<O>() << "'.";
  ...
  template <class O>
  std::string string_from_expression(const table__rowid_t<O> &rid,
                                     bool noTableName) const {
    std::stringstream ss;
    if (!noTableName) {
      ss << "'" << this->impl.template find_table_name<O>() << "'.";
  ...
  template <class T>
  std::string
  string_from_expression(const aggregate_functions::group_concat_double_t<T> &f,
                         bool noTableName) const {
    std::stringstream ss;
  ...
  template <class T>
  std::string
  string_from_expression(const aggregate_functions::group_concat_single_t<T> &f,
                         bool noTableName) const {
    std::stringstream ss;
    auto expr = this->string_from_expression(f.arg, noTableName);
  ...
  template <class L, class R, class... Ds>
  std::string string_from_expression(const binary_operator<L, R, Ds...> &f,
                                     bool noTableName) const {
    std::stringstream ss;
  ...
  template <class T>
  std::string string_from_expression(const aggregate_functions::min_t<T> &f,
                                     bool noTableName) const {
    std::stringstream ss;
    auto expr = this->string_from_expression(f.arg, noTableName);
    ss << static_cast<std::string>(f) << "(" << expr << ")";
  ...
  template <class T>
  std::string string_from_expression(const aggregate_functions::max_t<T> &f,
                                     bool noTableName) const {
    std::stringstream ss;
    auto expr = this->string_from_expression(f.arg, noTableName);
    ss << static_cast<std::string>(f) << "(" << expr << ")";
  ...
  template <class T>
  std::string string_from_expression(const aggregate_functions::total_t<T> &f,
                                     bool noTableName) const {
    std::stringstream ss;
    auto expr = this->string_from_expression(f.arg, noTableName);
    ss << static_cast<std::string>(f) << "(" << expr << ")";
  ...
  template <class T>
  std::string string_from_expression(const aggregate_functions::sum_t<T> &f,
                                     bool noTableName) const {
    std::stringstream ss;
    auto expr = this->string_from_expression(f.arg, noTableName);
    ss << static_cast<std::string>(f) << "(" << expr << ")";
  ...
  template <class T>
  std::string
  string_from_expression(const aggregate_functions::count_asterisk_t<T> &,
                         bool noTableName) const {
    return this->string_from_expression(
        aggregate_functions::count_asterisk_without_type{}, noTableName);
  }

  std::string string_from_expression(
      const aggregate_functions::count_asterisk_without_type &f,
      bool /*noTableName*/) const {
    std::stringstream ss;
  ...
  template <class T>
  std::string string_from_expression(const aggregate_functions::count_t<T> &f,
                                     bool noTableName) const {
    std::stringstream ss;
    auto expr = this->string_from_expression(f.arg, noTableName);
    ss << static_cast<std::string>(f) << "(" << expr << ")";
  ...
  template <class T>
  std::string string_from_expression(const aggregate_functions::avg_t<T> &a,
                                     bool noTableName) const {
    std::stringstream ss;
  ...
  template <class T>
  std::string string_from_expression(const distinct_t<T> &f,
                                     bool noTableName) const {
    std::stringstream ss;
  ...
  template <class T>
  std::string string_from_expression(const all_t<T> &f,
                                     bool noTableName) const {
    std::stringstream ss;
    auto expr = this->string_from_expression(f.t, noTableName);
    ss << static_cast<std::string>(f) << "(" << expr << ") ";
  ...
  std::string string_from_expression(const column_pointer<T, F> &c,
                                     bool noTableName) const {
    std::stringstream ss;
    if (!noTableName) {
  ...
  get_column_names(const internal::asterisk_t<T> &) const {
    std::vector<std::string> res;
    res.push_back("*");
    return res;
  }
  ...
  template <class... Args>
  std::vector<std::string>
  get_column_names(const internal::columns_t<Args...> &cols) const {
    std::vector<std::string> columnNames;
   ...
   */
  template <class T, class... Args>
  std::string string_from_expression(const internal::select_t<T, Args...> &sel,
                                     bool /*noTableName*/) const {
    std::stringstream ss;
    ...
    internal::join_iterator<Args...>()([&tableNamesSet, this](const auto &c) {
      ...
      auto crossJoinedTableName =
          this->impl.template find_table_name<cross_join_type>();
    ...
    if (!tableNamesSet.empty()) {
      ...
      std::vector<std::pair<std::string, std::string>> tableNames(
          tableNamesSet.begin(), tableNamesSet.end());
  ...
  // Common code for statements returning the whole content of a table:
  // get_all_t, get_all_pointer_t, get_all_optional_t.
  template <class T, class... Args>
  std::stringstream
  string_from_expression_impl_get_all(bool /*noTableName*/) const {
    std::stringstream ss;
    ss << "SELECT ";
    auto &impl = this->get_impl<T>();
  ...
  template <class T, class... Args>
  std::string string_from_expression(const get_all_t<T, Args...> &get,
                                     bool noTableName) const {
    std::stringstream ss =
        this->string_from_expression_impl_get_all<T>(noTableName);
    this->process_conditions(ss, get.conditions);
    return ss.str();
  }

  template <class T, class... Args>
  std::string string_from_expression(const get_all_pointer_t<T, Args...> &get,
                                     bool noTableName) const {
    std::stringstream ss =
        this->string_from_expression_impl_get_all<T>(noTableName);
    this->process_conditions(ss, get.conditions);
...
#ifdef SQLITE_ORM_OPTIONAL_SUPPORTED
  template <class T, class... Args>
  std::string string_from_expression(const get_all_optional_t<T, Args...> &get,
                                     bool noTableName) const {
    std::stringstream ss =
        this->string_from_expression_impl_get_all<T>(noTableName);
    this->process_conditions(ss, get.conditions);
  ...
  template <class... Args, class... Wargs>
  std::string
  string_from_expression(const update_all_t<set_t<Args...>, Wargs...> &upd,
                         bool /*noTableName*/) const {
    std::stringstream ss;
    ss << "UPDATE ";
  ...
  template <class T, class... Args>
  std::string string_from_expression(const remove_all_t<T, Args...> &rem,
                                     bool /*noTableName*/) const {
    auto &impl = this->get_impl<T>();
    std::stringstream ss;
  ...
  // Common code for statements with conditions: get_t, get_pointer_t,
  // get_optional_t.
  template <class T>
  ...
  template <class T, class... Ids>
  std::string string_from_expression(const get_t<T, Ids...> &,
                                     bool noTableName) const {
    return this->string_from_expression_impl_get<T>(noTableName);
  }

  template <class T, class... Ids>
  std::string string_from_expression(const get_pointer_t<T, Ids...> &,
                                     bool noTableName) const {
    return this->string_from_expression_impl_get<T>(noTableName);
  }
...
#ifdef SQLITE_ORM_OPTIONAL_SUPPORTED
  template <class T, class... Ids>
  std::string string_from_expression(const get_optional_t<T, Ids...> &,
                                     bool noTableName) const {
    return this->string_from_expression_impl_get<T>(noTableName);
  }
#endif // SQLITE_ORM_OPTIONAL_SUPPORTED
  ...
  template <class T>
  std::string string_from_expression(const update_t<T> &upd,
                                     bool /*noTableName*/) const {
  ...
  template <class T, class... Ids>
  std::string string_from_expression(const remove_t<T, Ids...> &,
                                     bool /*noTableName*/) const {
    auto &impl = this->get_impl<T>();
    std::stringstream ss;
    ss << "DELETE FROM '" << impl.table.name << "' ";
  ...
  template <class T, class... Cols>
  std::string string_from_expression(const insert_explicit<T, Cols...> &ins,
                                     bool /*noTableName*/) const {
  ...
  template <class T>
  std::string string_from_expression(const insert_t<T> &ins,
                                     bool /*noTableName*/) const {
    using expression_type = typename std::decay<decltype(ins)>::type;
    using object_type = typename expression_object_type<expression_type>::type;
    this->assert_mapped_type<object_type>();
    ...
    impl.table.for_each_column(
        [&impl, &columnNames, &compositeKeyColumnNames](auto &c) {
          if (impl.table._without_rowid ||
              !c.template has<constraints::primary_key_t<>>()) {
            auto it = std::find(compositeKeyColumnNames.begin(),
                                compositeKeyColumnNames.end(), c.name);
  ...
  template <class T>
  std::string string_from_expression(const replace_t<T> &rep,
                                     bool /*noTableName*/) const {
  ...
  template <class It>
  std::string string_from_expression(const replace_range_t<It> &rep,
                                     bool /*noTableName*/) const {
    using expression_type = typename std::decay<decltype(rep)>::type;
  ...
  template <class It>
  std::string string_from_expression(const insert_range_t<It> &ins,
                                     bool /*noTableName*/) const {
    using expression_type = typename std::decay<decltype(ins)>::type;
    using object_type = typename expression_type::object_type;
    auto &impl = this->get_impl<object_type>();
    ...
    auto valuesCount =
        static_cast<int>(std::distance(ins.range.first, ins.range.second));
    for (auto i = 0; i < valuesCount; ++i) {
      ss << valuesString;
      if (i < valuesCount - 1) {
        ss << ",";
  ...
  template <class T, class E>
  std::string string_from_expression(const conditions::cast_t<T, E> &c,
                                     bool noTableName) const {
    std::stringstream ss;
    ss << static_cast<std::string>(c) << " (";
    ss << this->string_from_expression(c.expression, noTableName) << " AS "
       << type_printer<T>().print() << ")";
    return ss.str();
  }

  template <class T>
  typename std::enable_if<is_base_of_template<T, compound_operator>::value,
                          std::string>::type
  ...
  template <class R, class T, class E, class... Args>
  std::string
  string_from_expression(const internal::simple_case_t<R, T, E, Args...> &c,
                         bool noTableName) const {
    std::stringstream ss;
    ss << "CASE ";
  ...
  template <class T>
  std::string string_from_expression(const conditions::is_null_t<T> &c,
                                     bool noTableName) const {
    std::stringstream ss;
    ss << this->string_from_expression(c.t, noTableName) << " "
       << static_cast<std::string>(c) << " ";
    return ss.str();
  }

  template <class T>
  std::string string_from_expression(const conditions::is_not_null_t<T> &c,
                                     bool noTableName) const {
    std::stringstream ss;
    ss << this->string_from_expression(c.t, noTableName) << " "
       << static_cast<std::string>(c) << " ";
  ...
  template <class C>
  std::string
  string_from_expression(const conditions::negated_condition_t<C> &c,
                         bool noTableName) const {
    std::stringstream ss;
    ss << static_cast<std::string>(c) << " ";
  ...
  template <class L, class R>
  std::string string_from_expression(const conditions::is_equal_t<L, R> &c,
                                     bool noTableName) const {
  ...
  template <class C>
  typename std::enable_if<
      is_base_of_template<C, conditions::binary_condition>::value,
      std::string>::type
  ...
  template <class T>
  std::string string_from_expression(const conditions::named_collate<T> &col,
                                     bool noTableName) const {
  ...
  template <class T>
  std::string string_from_expression(const conditions::collate_t<T> &col,
                                     bool noTableName) const {
    auto res = this->string_from_expression(col.expr, noTableName);
    return res + " " + static_cast<std::string>(col);
  }
  ...
  std::string string_from_expression(const conditions::in_t<L, A> &inCondition,
                                     bool noTableName) const {
    std::stringstream ss;
  ...
  template <class L, class E>
  std::string
  string_from_expression(const conditions::in_t<L, std::vector<E>> &inCondition,
                         bool noTableName) const {
    std::stringstream ss;
    auto leftString = this->string_from_expression(inCondition.l, noTableName);
    ss << leftString << " " << static_cast<std::string>(inCondition) << " ( ";
  ...
  template <class A, class T, class E>
  std::string string_from_expression(const conditions::like_t<A, T, E> &l,
                                     bool noTableName) const {
    std::stringstream ss;
  ...
  template <class A, class T>
  std::string string_from_expression(const conditions::glob_t<A, T> &l,
                                     bool noTableName) const {
    std::stringstream ss;
    ss << this->string_from_expression(l.arg, noTableName) << " ";
    ss << static_cast<std::string>(l) << " ";
  ...
  template <class A, class T>
  std::string string_from_expression(const conditions::between_t<A, T> &bw,
                                     bool noTableName) const {
    std::stringstream ss;
  ...
  template <class T>
  std::string string_from_expression(const conditions::exists_t<T> &e,
                                     bool noTableName) const {
    std::stringstream ss;
  ...
  template <class T>
  void process_join_constraint(std::stringstream &ss,
                               const conditions::on_t<T> &t) const {
    ss << static_cast<std::string>(t) << " "
       << this->string_from_expression(t.arg, false);
  }

  template <class F, class O>
  void process_join_constraint(std::stringstream &ss,
                               const conditions::using_t<F, O> &u) const {
    ss << static_cast<std::string>(u) << " ("
       << this->string_from_expression(u.column, true) << " )";
  }
  ...
  void process_single_condition(
      std::stringstream &ss,
      const conditions::limit_t<T, HO, OI, O> &limt) const {
  ...
  void process_single_condition(std::stringstream &ss,
                                const conditions::cross_join_t<O> &c) const {
    ss << static_cast<std::string>(c) << " ";
    ss << " '" << this->impl.template find_table_name<O>() << "'";
  }
  ...
  template <class O>
  void process_single_condition(std::stringstream &ss,
                                const conditions::natural_join_t<O> &c) const {
    ss << static_cast<std::string>(c) << " ";
    ss << " '" << this->impl.template find_table_name<O>() << "'";
  }
  ...
  template <class T, class O>
  void process_single_condition(std::stringstream &ss,
                                const conditions::inner_join_t<T, O> &l) const {
    ss << static_cast<std::string>(l) << " ";
    auto aliasString = alias_extractor<T>::get();
    ss << " '"
       << this->impl
              .template find_table_name<typename mapped_type_proxy<T>::type>()
       << "' ";
  ...
  template <class T, class O>
  void
  process_single_condition(std::stringstream &ss,
                           const conditions::left_outer_join_t<T, O> &l) const {
    ss << static_cast<std::string>(l) << " ";
    ss << " '" << this->impl.template find_table_name<T>() << "' ";
    this->process_join_constraint(ss, l.constraint);
  }
  ...
  template <class T, class O>
  void process_single_condition(std::stringstream &ss,
                                const conditions::left_join_t<T, O> &l) const {
    ss << static_cast<std::string>(l) << " ";
    ss << " '" << this->impl.template find_table_name<T>() << "' ";
    this->process_join_constraint(ss, l.constraint);
  }
  ...
  template <class T, class O>
  void process_single_condition(std::stringstream &ss,
                                const conditions::join_t<T, O> &l) const {
    ss << static_cast<std::string>(l) << " ";
    ss << " '" << this->impl.template find_table_name<T>() << "' ";
    this->process_join_constraint(ss, l.constraint);
  }
  ...
  template <class C>
  void process_single_condition(std::stringstream &ss,
                                const conditions::where_t<C> &w) const {
  ...
  void
  process_single_condition(std::stringstream &ss,
                           const conditions::order_by_t<O> &orderBy) const {
    ss << static_cast<std::string>(orderBy) << " ";
  ...
  void process_single_condition(
      std::stringstream &ss,
      const conditions::group_by_t<Args...> &groupBy) const {
    std::vector<std::string> expressions;
  ...
  template <class T>
  void process_single_condition(std::stringstream &ss,
                                const conditions::having_t<T> &hav) const {
    ss << static_cast<std::string>(hav) << " ";
    ss << this->string_from_expression(hav.t, false) << " ";
  }

  template <class... Args>
  void process_conditions(std::stringstream &ss,
                          const std::tuple<Args...> &args) const {
    iterate_tuple(
        args, [this, &ss](auto &v) { this->process_single_condition(ss, v); });
  }
...
protected:
  ...
  parse_table_name(const T &) const {
    return {};
  }

  template <class F, class O>
  std::set<std::pair<std::string, std::string>>
  parse_table_name(F O::*, std::string alias = {}) const {
    return {std::make_pair(this->impl.template find_table_name<O>(),
                           std::move(alias))};
  }
  ...
  template <class T>
  std::set<std::pair<std::string, std::string>> parse_table_name(
      const aggregate_functions::group_concat_double_t<T> &f) const {
  ...
  template <class T>
  std::set<std::pair<std::string, std::string>> parse_table_name(
      const aggregate_functions::group_concat_single_t<T> &f) const {
    return this->parse_table_name(f.arg);
  }
  ...
  template <class R, class S, class... Args>
  std::set<std::pair<std::string, std::string>> parse_table_name(
      const core_functions::core_function_t<R, S, Args...> &f) const {
  ...
  template <class T>
  std::set<std::pair<std::string, std::string>>
  parse_table_name(const distinct_t<T> &f) const {
    return this->parse_table_name(f.t);
  }

  template <class T>
  std::set<std::pair<std::string, std::string>>
  parse_table_name(const all_t<T> &f) const {
    return this->parse_table_name(f.t);
  }
  ...
  template <class T, class F>
  std::set<std::pair<std::string, std::string>>
  parse_table_name(const column_pointer<T, F> &) const {
    std::set<std::pair<std::string, std::string>> res;
  ...
  template <class T, class C>
  std::set<std::pair<std::string, std::string>>
  parse_table_name(const alias_column_t<T, C> &a) const {
  ...
  std::set<std::pair<std::string, std::string>> parse_table_name(
      const aggregate_functions::count_asterisk_without_type &) const {
    return {};
  }

  template <class T>
  std::set<std::pair<std::string, std::string>>
  parse_table_name(const asterisk_t<T> &) const {
    auto tableName = this->impl.template find_table_name<T>();
    return {std::make_pair(std::move(tableName), "")};
  }
  ...
  template <class T, class E>
  std::set<std::pair<std::string, std::string>>
  parse_table_name(const conditions::cast_t<T, E> &c) const {
  ...
  template <class A, class T, class E>
  std::set<std::pair<std::string, std::string>>
  parse_table_name(const conditions::like_t<A, T, E> &l) const {
    std::set<std::pair<std::string, std::string>> res;
  ...
  template <class A, class T>
  std::set<std::pair<std::string, std::string>>
  parse_table_name(const conditions::glob_t<A, T> &l) const {
    std::set<std::pair<std::string, std::string>> res;
    auto argTableNames = this->parse_table_name(l.arg);
    res.insert(argTableNames.begin(), argTableNames.end());
    auto patternTableNames = this->parse_table_name(l.pattern);
  ...
  template <class T, class E>
  std::set<std::pair<std::string, std::string>>
  parse_table_name(const as_t<T, E> &a) const {
  ...
  std::string group_concat_internal(F O::*m, std::unique_ptr<std::string> y,
    ...
    if (y) {
      rows = this->select(sqlite_orm::group_concat(m, move(*y)),
...
public:
  ...
  template <class F, class O, class... Args, class Tuple = std::tuple<Args...>,
            typename sfinae = typename std::enable_if<
                std::tuple_size<Tuple>::value >= 1>::type>
  ...
  std::string group_concat(F O::*m, const char *y, Args &&... args) {
    ...
    return this->group_concat_internal(m, move(str),
                                       std::forward<Args>(args)...);
  }
   ...
   */
  template <class O> std::string dump(const O &o) {
    this->assert_mapped_type<O>();
...
protected:
  ...
  sync_schema_result
  sync_table(storage_impl<internal::index_t<Cols...>, Tss...> *impl,
             sqlite3 *db, bool) {
    auto res = sync_schema_result::already_in_sync;
    std::stringstream ss;
    ss << "CREATE ";
  ...
  sync_schema_result sync_table(storage_impl<table_t<Cs...>, Tss...> *impl,
    ...
    if (schema_stat != decltype(schema_stat)::already_in_sync) {
      if (schema_stat == decltype(schema_stat)::new_table_created) {
        ...
        if (schema_stat == sync_schema_result::old_columns_removed ||
          ...
          impl->get_remove_add_columns(columnsToAdd, storageTableInfo,
                                       dbTableInfo);
...
public:
  ...
  prepare(get_all_pointer_t<T, Args...> get) {
    auto con = this->get_connection();
    sqlite3_stmt *stmt;
    auto db = con.get();
    auto query = this->string_from_expression(get, false);
...
#ifdef SQLITE_ORM_OPTIONAL_SUPPORTED
  template <class T, class... Args>
  prepared_statement_t<get_all_optional_t<T, Args...>>
  prepare(get_all_optional_t<T, Args...> get) {
    auto con = this->get_connection();
    sqlite3_stmt *stmt;
    auto db = con.get();
    auto query = this->string_from_expression(get, false);
  ...
  prepare(get_optional_t<T, Ids...> g) {
    auto con = this->get_connection();
    sqlite3_stmt *stmt;
    auto db = con.get();
    auto query = this->string_from_expression(g, false);
  ...
  execute(const prepared_statement_t<insert_explicit<T, Cols...>> &statement) {
    ...
    sqlite3_reset(stmt);
    iterate_tuple(statement.t.columns.columns, [&o, &index, &stmt, &impl,
                                                db](auto &m) {
      ...
      const field_type *value =
          impl.table.template get_object_field_pointer<field_type>(o, m);
      if (SQLITE_OK !=
      ...
      impl.table.for_each_column([&o, &index, &stmt, db](auto &c) {
        ...
        if (c.member_pointer) {
          if (SQLITE_OK != statement_binder<field_type>().bind(
            ...
            throw std::system_error(
                std::error_code(sqlite3_errcode(db),
                                get_sqlite_error_category()),
                sqlite3_errmsg(db));
          }
        } else {
  ...
  int64 execute(const prepared_statement_t<insert_t<T>> &statement) {
    ...
    auto compositeKeyColumnNames = impl.table.composite_key_columns_names();
    sqlite3_reset(stmt);
    impl.table.for_each_column([&o, &index, &stmt, &impl,
                                &compositeKeyColumnNames, db](auto &c) {
      if (impl.table._without_rowid ||
          !c.template has<constraints::primary_key_t<>>()) {
        auto it = std::find(compositeKeyColumnNames.begin(),
                            compositeKeyColumnNames.end(), c.name);
  ...
  template <class T, class... Args>
  std::vector<T>
  execute(const prepared_statement_t<get_all_t<T, Args...>> &statement) {
    auto &impl = this->get_impl<T>();
    auto con = this->get_connection();
    auto db = con.get();
    auto stmt = statement.stmt;
  ...
  template <class T, class... Args>
  std::vector<std::unique_ptr<T>> execute(
      const prepared_statement_t<get_all_pointer_t<T, Args...>> &statement) {
    auto &impl = this->get_impl<T>();
    auto con = this->get_connection();
    auto db = con.get();
    auto stmt = statement.stmt;
...
#ifdef SQLITE_ORM_OPTIONAL_SUPPORTED
  template <class T, class... Args>
  std::vector<std::optional<T>> execute(
      const prepared_statement_t<get_all_optional_t<T, Args...>> &statement) {
    auto &impl = this->get_impl<T>();
    auto con = this->get_connection();
    auto db = con.get();
    auto stmt = statement.stmt;
...
template <class... Ts>
internal::storage_t<Ts...> make_storage(const std::string &filename,
                                        Ts... tables) {
...
#include <utility>    //  std::pair
  ...
  template <class C> struct node_tuple<conditions::where_t<C>, void> {
    using node_type = conditions::where_t<C>;
    using type = typename node_tuple<C>::type;
  };

  template <class T>
  struct node_tuple<T, typename std::enable_if<is_base_of_template<
                           T, conditions::binary_condition>::value>::type> {
    using node_type = T;
  ...
  template <class L, class A> struct node_tuple<conditions::in_t<L, A>, void> {
    using node_type = conditions::in_t<L, A>;
  ...
  template <class T> struct node_tuple<conditions::having_t<T>, void> {
    using node_type = conditions::having_t<T>;
    using type = typename node_tuple<T>::type;
  };
  ...
  template <class T> struct node_tuple<conditions::exists_t<T>, void> {
    using node_type = conditions::exists_t<T>;
    using type = typename node_tuple<T>::type;
  };
  ...
  template <class T> struct node_tuple<conditions::named_collate<T>, void> {
    using node_type = conditions::named_collate<T>;
    using type = typename node_tuple<T>::type;
  };

  template <class T> struct node_tuple<conditions::is_null_t<T>, void> {
    using node_type = conditions::is_null_t<T>;
    using type = typename node_tuple<T>::type;
  };

  template <class T> struct node_tuple<conditions::is_not_null_t<T>, void> {
    using node_type = conditions::is_not_null_t<T>;
    using type = typename node_tuple<T>::type;
  };
  ...
  template <class T> struct node_tuple<conditions::on_t<T>, void> {
    using node_type = conditions::on_t<T>;
    using type = typename node_tuple<T>::type;
  };
  ...
  struct node_tuple<conditions::limit_t<T, true, false, O>, void> {
    using node_type = conditions::limit_t<T, true, false, O>;
    using type = typename conc_tuple<typename node_tuple<T>::type,
  ...
  struct node_tuple<conditions::limit_t<T, true, true, O>, void> {
    using node_type = conditions::limit_t<T, true, true, O>;
    using type = typename conc_tuple<typename node_tuple<O>::type,
...
const auto &get(const internal::prepared_statement_t<T> &statement) {
  ...
  internal::iterate_ast(statement.t, [&result, &index](auto &node) {
    ...
    if (index == N) {
      internal::static_if<std::is_same<result_tupe, node_type>{}>(
          [](auto &result, auto &node) {
            result = const_cast<
                typename std::remove_reference<decltype(result)>::type>(&node);
          })(result, node);
    }
  });
...
template <class R, class S, class... Args>
struct statement_serializator<core_functions::core_function_t<R, S, Args...>,
                              void> {
  ...
  std::string operator()(const statement_type &c) const {
    ...
    using args_type = typename std::decay<decltype(c)>::type::args_type;
    args.reserve(std::tuple_size<args_type>::value);
    iterate_tuple(c.args, [&args](auto &v) { args.push_back(serialize(v)); });
    for (size_t i = 0; i < args.size(); ++i) {
      ss << args[i];
      if (i < args.size() - 1) {
        ss << ", ";
...
template <> struct statement_serializator<constraints::unique_t, void> {
